...
class Test_pc(object):
    ...
    def test_insertar_posicion_aleatoria(self):
        ...
        assert (
            tablero_recibido == tablero_esperado
        ).all(), "pc.insertar_posicion() no inserta la posicion esperada"
    ...
    def test_insertar_posicion_condicionada(self):
        ...
        def verificar_assert(
            ...
            for (
                ...
                assert (
                    tablero_recibido == tablero_esperado
                ).all(), "pc.insertar_posicion() ({} condicionada) no inserta la posicion esperada".format(
                    nombre_test
                )
                print(
                    "SUCCEEDED pc.insertar_posicion() ({} condicionada)".format(
                        nombre_test
                    )
                )
        ...
        verificar_assert(
            "Diagonal principal",
            "np.fill_diagonal(tablero_recibido, posicion_posible)",
            "np.fill_diagonal(tablero_esperado, posicion_esperada)",
            False,
        )
        verificar_assert(
            "Diagonal secundaria",
            "np.fill_diagonal(np.fliplr(tablero_recibido), posicion_posible)",
            "np.fill_diagonal(np.fliplr(tablero_esperado), posicion_esperada)",
            False,
        )

        for i in range(3):
            verificar_assert(
                "Horizontal {}".format(i + 1),
                "tablero_recibido[{}] = posicion_posible".format(i),
                "tablero_esperado[{}] = posicion_esperada".format(i),
                True,
            )
            verificar_assert(
                "Vertical {}".format(i + 1),
    ...
    def test_get_simbolo(self):
        ...
        ), "pc.get_simbolo() no devuelve el simbolo esperado"
